...
public class DashboardPlayerDAO extends ConnectDB {
    ...
    public int gettotalMeetingInMonth(int month) {
        sql = " SELECT  COUNT(*) AS MeetingCount  FROM   [dbo].[MeetingSchedule]\n"
                + "WHERE    YEAR(MeetingDate) = YEAR(GETDATE()) and     MONTH(MeetingDate) = " + month
                + "GROUP BY    MONTH(MeetingDate)";
        try {
            con = this.openConnection();
            st = con.prepareStatement(sql);
            rs = st.executeQuery();
    ...
    public int gettotalTreatmentInMonth(int month) {
        sql = " SELECT \n"
                ...
                + "WHERE \n"
                + "    DATEPART(YEAR, TreatmentDate) = YEAR(GETDATE()) and DATEPART(MONTH, TreatmentDate) = " + month
                + "GROUP BY \n"
